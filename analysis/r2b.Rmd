...
(Shrunk R^2 = $1 - \exp(fash.output)*\frac{n-p-1}{n-1}$, because $R^2=1-\frac{\sum(y_i-\hat y_i)^2}{\sum(y_i-\bar y)^2}=1-\frac{\sum(y_i-\hat y_i)^2/(n-p-1)}{\sum(y_i-\bar y)^2/(n-1)}*\frac{n-p-1}{n-1}=1-(1-adjR^2)*\frac{n-p-1}{n-1}$)(note: this is the old idea which introduces bias to R^2 when multiplying $\frac{n-p-1}{n-1}$ so I discard this method.)
