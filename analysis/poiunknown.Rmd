...
mat=matrix(result$est$smashgen.smashu.out$m.est,nrow=6,byrow = T)
par(mfrow=c(3,2))
...
Obviously, the present of 'outliers' makes the smashgen diverge. We now try the 'robust' version of smashgen, which sets the finest level wavelet cofficients to 0.

```{r}
result.robust=simu_study(m,1,nsimu = 6,return.x = T,robust = T)
mat=matrix(result.robust$est$smashgen.smashu.out$m.est,nrow=6,byrow = T)
par(mfrow=c(3,2))
plot(result.robust$est$x[1,],col='gray80')
lines(exp(m),col='gray30')
lines(mat[1,],col=4)

plot(result.robust$est$x[2,],col='gray80')
lines(exp(m),col='gray30')
lines(mat[2,],col=4)

plot(result.robust$est$x[3,],col='gray80')
lines(exp(m),col='gray30')
lines(mat[3,],col=4)

plot(result.robust$est$x[4,],col='gray80')
lines(exp(m),col='gray30')
lines(mat1[4,],col=4)

plot(result.robust$est$x[5,],col='gray80')
lines(exp(m),col='gray30')
lines(mat[5,],col=4)

plot(result.robust$est$x[6,],col='gray80')
lines(exp(m),col='gray30')
lines(mat[6,],col=4)
```

It seems that this does no help with the issue.
