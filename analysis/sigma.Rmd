...
for(i in 1:100){
  ...
  smashs[i]=sigma_est(y,mu,st,'smash')
  moment[i]=sigma_est(y,mu,st,'moment')
  ols[i]=sigma_est(y,mu,st,'ols')
}
boxplot(data.frame(wls=wls,ols=ols,smash_s=smashs,moment=moment))
...
boxplot(data.frame(ols.sst=results$ols.sst,moment.sst=results$moment.sst,
                   ...
                   smashu.sst=results$smashu.sst,rmad.sst=results$rmad.sst))
abline(h=0,lty=2)
```
