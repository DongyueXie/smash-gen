...
Though mle estimate is unbiased, it's variance is large. Thus, using robust estimator is possible a better way. We consider Huber M-estimator. However, since the distribution of $\tilde Z_t$ is highly skewed, Huber M estimator is not very appropriate in this context.
...
To figure out how the improvements of variance estimation help with the mean estimaiton, we also calculate the MSE of mean estimation using the estimated variance from corresponding methods. To ficilatate the comparsions, the results from smash with true variance(*smashtrue*) are added.
...
```
Tough the estimate of total sd are more accurate for the first 5 methods(but not significantly better), this did not help with the mean estimation a lot. 
...
The variance estimaitons using smash indeed are not as good as the first five methods, while the mean estimations are not significantly worse but have larger variance.
